...
pub async fn run(
    ...
    loop {
        ...
        let expert_handler = expert_handler.clone();

        tokio::spawn(async move {
            match session::run_session(socket, upstream_addr, registry, handler, shard_handler, speculative_handler, audit_handler, expert_handler).await {
                Ok(()) => {
